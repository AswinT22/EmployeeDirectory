...
public class BuildResponseAdminService {
	...
	public ResponseEntity<Object> add(EmployeeModel employees) {
		...
		try {
			...
			if(!service.add(employees)) {
						...
						empInfo=new EmployeeInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.FAILED"),EmployeeDTO.modelOf(employees));
						return empInfo.buildEmployeeInfo();		
					}
					
					
					empInfo=new EmployeeInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.ALL_CREATED"),EmployeeDTO.modelOf(employees));
			...
			List<EmployeeDTO> employeeDTOs= 
				...
				logger.info("Employee Input List: {}", employeeDTOs);
					...
					if(failedUpdate.size()>0) {
						...
						empInfo=new EmployeeInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.PARTIAL_Success"),failedUpdate);
						
						if(failedUpdate.size()==employeeDTOs.size())
							empInfo=new EmployeeInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.FAILED"),failedUpdate);
					...
					empInfo=new EmployeeInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.ALL_CREATED"),employeeDTOs);
	...
	public ResponseEntity<Object>  processQuery(QueryModel query){
		...
		try {
				...
				EmployeeInformation empInfo=new EmployeeInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.ALL_UPDATED"),empListDTOS);
	...
	public ResponseEntity<Object>  processQueries(QueryModel[] queries){
		...
		try {
				...
				if(failedUpdate.size()>0) {
					...
					updateInfo=new UpdateInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.PARTIAL_Success"),failedUpdate);
					
					if(failedUpdate.size()==queries.length)
						updateInfo=new UpdateInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.FAILED"),failedUpdate);
				...
				updateInfo=new UpdateInformation(HttpStatus.ACCEPTED,config.getMessage("DirectoryService.ALL_QUERY_UPDATED"),Arrays.asList(queries));
